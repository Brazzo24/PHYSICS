...
def main():
    ...
    # Augment system
    m, c_inter, k_inter, C_ext, K_ext = augment_with_branch(
        m, c_inter, k_inter, branch_connection,
        m_branch, c_branch, k_branch,
        c_to_ground=c_to_ground
    )


    N = len(m)

    f_n, eigvecs, M_free, K_free = free_vibration_analysis_free_chain(
            m, k_inter, C_ext=C_ext, K_ext=K_ext
            )
    ...
    if COMPUTE_FORCED_RESPONSE:
        response = forced_response_postprocessing(m, c_inter, k_inter, f_vals, F_ext, C_ext=C_ext, K_ext=K_ext)
    ...
    # Index of the branch mass
    idx_branch = len(m) - 1

    # Ground damping value (same as used in augment_with_branch)
    #c_to_ground = 0.5  # adjust if changed

    # Velocity of branch mass (complex)
    V_branch = V_vals[idx_branch, :]

    # Power dissipated: Re{c * v * v*} = c * |v|^2
    P_damp_ground = c_to_ground * np.abs(V_branch)**2

    # Plot
    plt.figure()
    plt.plot(f_vals, P_damp_ground)
    plt.xlabel("Frequency (Hz)")
    plt.ylabel("Power Dissipated (Nm/s)")
    plt.title("Power Dissipated by Ground Damper at Branch Mass")
    plt.grid(True)
    plt.show()

    plt.figure()
    plt.plot(f_vals, np.abs(X_vals[idx_branch, :]))
    plt.xlabel("Frequency (Hz)")
    plt.ylabel("Displacement Magnitude (rad)")
    plt.title("Displacement of Branch Mass")
    plt.grid(True)
    plt.show()
