...
def main():
    ...
    c_inter = np.array([0.05, 0.05, 0.05, 0.05, 0.05, 0.05, 0.05, 0.05, 0.05, 0.05]) # Nm.s/rad
    ...
    k_inter = np.array([2.34e4, 1.62e5, 1.1e3, 1.10e5, 1.10e5,
                    2.72e4, 4.97e3, 2.73e2, 8.57e2, 1e-3]) # Nm/rad
    ...
    k_inter[8] *= 1.8
       ...
       # Add branch mass at the end
    m_branch = 0.005
    f_target = 24  # Hz
    k_branch = (2 * np.pi * f_target)**2 * m_branch
    c_branch = 2 * np.sqrt(m_branch * k_branch) * 0.15  # damping ratio

    m = np.append(m, m_branch)
    branch_connection = (8, len(m) - 1)
    ...
    f_n, eigvecs, M_free, K_free = free_vibration_analysis_with_branch(
        m, k_inter,
    ...
    if COMPUTE_FORCED_RESPONSE:
        response = forced_response_postprocessing(
        m, c_inter, k_inter, f_vals, F_ext,
        branch_connection=branch_connection,
        k_branch=k_branch,
        c_branch=c_branch
        )
