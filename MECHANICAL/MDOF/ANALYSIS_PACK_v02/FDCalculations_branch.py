...
def build_free_chain_matrices_with_branch(m, c_inter, k_inter, branch_connection=None, k_branch=None, c_branch=None):
    """
    Extended version of build_free_chain_matrices that supports a single
    branch connection (like a TMD or side detuner).

    Parameters:
    - m: inertia array (including branch mass if added)
    - c_inter, k_inter: standard damping/stiffness arrays for linear chain (length N-1)
    - branch_connection: (i, j) tuple with DOF indices for the branch (e.g., (8, N))
    - k_branch, c_branch: stiffness and damping for the branch
...
def compute_forced_response_with_branch(m, c_inter, k_inter, f_vals, F_ext, branch_connection=None, k_branch=None, c_branch=None):
    N = len(m)
    num_points = len(f_vals)
    X = np.zeros((N, num_points), dtype=complex)
    F_bound = np.zeros(num_points, dtype=complex)

    M, C, K = build_free_chain_matrices_with_branch(m, c_inter, k_inter, branch_connection, k_branch, c_branch)
...
def forced_response_postprocessing(m, c_inter, k_inter, f_vals, F_ext, branch_connection=None, k_branch=None, c_branch=None):
    N = len(m)
    num_points = len(f_vals)
...
def free_vibration_analysis_with_branch(m, k_inter, branch_connection=None, k_branch=None, c_branch=None):
    dummy_damping = np.zeros(len(m) - 1)  # <-- FIXED here
    M, _, K = build_free_chain_matrices_with_branch(m, dummy_damping, k_inter, branch_connection, k_branch, c_branch)
    eigvals, eigvecs = eigh(K, M)
    omega_n = np.sqrt(np.maximum(eigvals, 0))
    f_n = omega_n / (2 * np.pi)
    return f_n, eigvecs, M, K
