...
================================================
Physics: the lean angle is governed by a damped 2nd-order
response to the equilibrium demand φ_eq = arctan(v²κ/g).

This is equivalent to a full nonlinear roll model in the limit
where the rider tightly tracks the balance point (which real
motorcycles do within ~0.3–0.5 s bandwidth). The tyre and
load-transfer physics are then computed from the resulting
phi(t), phi_dot(t), phi_ddot(t).
"""
...
p = dict(
    ...
    Ix     = 28.0,    # roll inertia  [kg·m²]
    h      = 0.62,    # CoM height  [m]
    L      = 1.45,    # wheelbase  [m]
    Lf     = 0.72,    # CoM–front distance  [m]
    v      = 18.0,    # speed  [m/s]  (~65 km/h)
    g      = 9.81,

    # Roll bandwidth: ω_n ≈ 10 rad/s, ζ ≈ 0.7  (typical sport bike)
    omega_n = 10.0,   # natural frequency of lean response  [rad/s]
...
# ─────────────────────────────────────────────────────────────────
# ODE:  state = [φ, φ̇]
# 2nd-order tracking of φ_eq with bandwidth (ω_n, ζ)
# ─────────────────────────────────────────────────────────────────
def equations(t, state, p):
    ...
    phi_eq = np.arctan2(v**2 * kappa, g)

    # Closed-loop roll dynamics (2nd order)
...
# ─────────────────────────────────────────────────────────────────
# POST-PROCESS — tyre forces, normal loads
# ─────────────────────────────────────────────────────────────────
m   = p['m'];  g  = p['g'];  h  = p['h']
...
epsilon_ra = np.arctan2(h_rc_f - h_rc_r, L)   # negative (front lower than rear)

# Signals over time
kappa      = np.array([curvature_profile(ti, p['R_turn']) for ti in t])
phi_eq_arr = np.arctan2(v**2 * kappa, g)
phi_ddot   = np.gradient(phi_dot, t)      # numerical derivative

# ── (b) Lateral couple through roll-axis moment arm ───────────────
# The centripetal force m·v²·κ acts at CoM height h.
# Its moment about the roll axis = m·v²·κ·(h − h_ra)
# Reacted over wheelbase: rear increases, front decreases.
M_pitch_lat = m * v**2 * kappa * (h - h_ra)

# ── (c) Roll-axis inertial couple ─────────────────────────────────
# During φ̈ transients the inertia Ix·φ̈ has a component along the
# roll axis: M_ra = Ix·φ̈·sin(ε_ra)
# ε_ra < 0 (front lower) → during chicane entry (φ̈ > 0 going left)
# M_ra < 0 → additional front load; opposite during exit.
M_rollaxis = Ix * phi_ddot * np.sin(epsilon_ra)

# ── Normal loads ──────────────────────────────────────────────────
Fzf = np.clip(m*g*Lr/L - (M_pitch_lat + M_rollaxis) / L, 0, None)
Fzr = np.clip(m*g*Lf/L + (M_pitch_lat + M_rollaxis) / L, 0, None)
...
fig.suptitle(
    ...
    f'R = {p["R_turn"]} m  |  m = {m} kg  |  ω_n = {p["omega_n"]} rad/s  ζ = {p["zeta"]}',
...
# Mark peak roll rate (chicane transition)
...
ax_rate.annotate(f'peak {np.degrees(phi_dot[idx_peak]):.1f}°/s',
                 ...
                 xytext=(t[idx_peak]+0.3, np.degrees(phi_dot[idx_peak])*0.7),
                 fontsize=8, color='#C04828',
                 arrowprops=dict(arrowstyle='->', color='#C04828', lw=0.8))
...
print(f"  Max lean angle    : {np.max(np.abs(lean_deg)):.1f}°"
      f"  (equilibrium target: {np.max(np.abs(np.degrees(phi_eq_arr))):.1f}°)")
...
print(f"  Rear  Fz range    : {np.min(Fzr):.0f} – {np.max(Fzr):.0f} N"
      f"  (static = {Fzr_s:.0f} N)")
print(f"  Max front |Fy|    : {np.max(np.abs(Fyf)):.0f} N")
print(f"  Max rear  |Fy|    : {np.max(np.abs(Fyr)):.0f} N")
